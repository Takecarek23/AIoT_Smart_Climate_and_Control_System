...
```mermaid
graph TD
    %% Định nghĩa các Task trong Subgraph
    subgraph FreeRTOS_Tasks [FreeRTOS Tasks]
        direction TB
        T1[Task Sensor<br/>Đọc DHT20]
        T2[Task WebServer<br/>UI & WebSocket]
        T3[Task CoreIoT<br/>MQTT/Cloud]
        T4[Task NeoPixel<br/>Hiển thị màu]
        T5[Task Fan Control<br/>Điều khiển động cơ]
        T6[Task AI/Logic<br/>Xử lý tự động]
    end
    ...
    %% Kết nối Người dùng & Cloud
    User <-->|Browser| T2
    Cloud <-->|Internet| T3

    %% Kết nối luồng dữ liệu giữa các Task
    T1 -->|Global Vars| T2
    T1 -->|Global Vars| T3
    T1 -->|Global Vars| T4
    T1 -->|Global Vars| T6
